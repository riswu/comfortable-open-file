...
export default class ComfortableOpenFileModel extends EventEmitter {
  constructor() {
    super()
    this.basePath = '';
    this.state = {
  ...
  static async isValidPath(somePath) {
    if (['', '~', '.', '..'].some((v) => somePath.startsWith(v + stdPath.sep))) return true;
    if (somePath.endsWith(':')) return false;
