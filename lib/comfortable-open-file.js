...
class ComfortableOpenFileModel {
  ...
  static filter(paths, query) {
    ...
    if (isDirPath(query)) return paths.filter((path) => !path.tag || path.tag === 'create-directory');
  ...
  static fetchPaths(dirPath) {
    ...
    const fetchedPaths = fileNames.map((fileName) => {
...
export default {
  ...
  attach() {
    ...
    const initDir = activeFileDirPath ? activeFileDirPath : stdPath.dirname(resolveTilde('~/hoge.txt'));
    this.comfortableOpenFileModel.setBasePath(initDir);
    this.comfortableOpenFileModel.moveDir(initDir);
    if (!activeFileDirPath) this.comfortableOpenFileModel.setQuery('~/hoge.txt');
