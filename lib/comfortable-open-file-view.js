...
export default class ComfortableOpenFileView {
  ...
  update(props) {
    const state = {};
    if (props.hasOwnProperty('query') && props.query !== this.query) {
      this.query = props.query;
      state.query = props.query;
    }
    if (props.hasOwnProperty('paths') && props.paths !== this.paths) {
      this.paths = props.paths;
      state.items = props.paths;
    }
    if (props.hasOwnProperty('error') && props.error && props.error.message !== this.errorMessage) {
      this.errorMessage = props.error.message;
      state.errorMessage = props.error.message;
      state.emptyMessage = '';
    } else if (!props.hasOwnProperty('error') && this.errorMessage) {
      this.errorMessage = '';
      state.errorMessage = '';
      state.emptyMessage = 'No matches found';
    }
    this.selectListView.update(state);
  }
